...
CREATE FUNCTION public.spend_video_credit(p_org_id uuid, p_profile_id uuid, p_request_id uuid DEFAULT NULL::uuid, p_reason text DEFAULT 'video_generation'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  VALUES (
    ...
    v_bucket,
    -1,
    COALESCE(p_reason, 'video_generation')
  )
...
CREATE TABLE public.credit_transactions (
    ...
    reason text DEFAULT 'video_generation'::text NOT NULL,
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
...
CREATE TABLE public.file_attachments (
    ...
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT file_attachments_entity_type_check CHECK ((entity_type = ANY (ARRAY['video_project'::text, 'video_asset'::text, 'video_generation'::text, 'render_job'::text, 'video_output'::text, 'organization'::text, 'profile'::text]))),
